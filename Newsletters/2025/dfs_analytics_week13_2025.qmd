...
Welcome to the first BuildMeUpButterCut PGA fantasy league update, where we’re teeing off in style for Masters week—Augusta dreams and green jacket schemes! Our league has been on fire with close to 9500 birdies to date, but we’ve also shanked \~6000 bogeys. With an average FPPG of 66.2 this season, some of us are channeling Justin Thomas’s clutch vibes, while others are pulling a 'go-for-broke' à la Phil Mickelson.

Here’s hoping we all survive Amen Corner with our dignity intact!
...
The pot this year will be given to the Manager with top 10 highest scores. League standings are somewhat volatile as we have raced to fill scoring slots. Nearly everyone has 10 scores under their belt at this point and the cumulative 10-score rankings are shown below by event.
...
"JT's not-so-good golf is a lot better than my not-so-good golf." - Max Homa (also probably Dana who picked JT the most)
...
value_golfers <- dbGetQuery(conn, "
          ...
          SELECT * FROM CTE2 WHERE EVENT_COUNT > 3 LIMIT 10
           ")
t <- value_golfers %>% 
  mutate(event_count_bin = case_when(event_count <= 6 ~ '3-6',
                                                          event_count <= 9 & event_count > 6 ~ '7-9',
                                                          event_count >= 10 ~ '10+')) 
```
...
ggplot(t, aes(y=reorder(player_name, avg_value), x = avg_value, col = factor(event_count_bin)))+
  ...
  theme(
    ...
    legend.position.inside = c(.4,1.25),
  ...
  guides(color = guide_legend(override.aes = list(size = 20), nrow = 4), size = guide_legend(nrow = 4))
```

## 6 - Missed Cut Selections

The number of golfers selected that have been cut, withdrawn, DQ'd, etc. in 2025. If you're at the top of this list, you either suck or are very unlucky. Probably the latter.
