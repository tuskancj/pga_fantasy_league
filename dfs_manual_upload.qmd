...
df_hist_current_year <- df_hist %>% filter(tour == 'pga', calendar_year == as.numeric(format(Sys.Date(), "%Y"))) %>% arrange(desc(date)) %>% mutate(row_number = row_number())
df_hist_current_year
```
...
```{r}
events_played_to_date <- dbGetQuery(conn, "select * from event where calendar_year = 2025") %>% arrange(desc(date)) %>% mutate(row_number = row_number())
events_played_to_date
```
...
df <- dbGetQuery(conn, "
          ...
          ORDER BY
           ...
           ")
# df
last_event <- df %>% group_by(event_name, event_id, id_event, date) %>% summarize(entry_count = length(username), .groups = 'drop') %>% arrange(desc(date)) %>% head(1)
last_event
```
```{r}
previous_contest <- length(unique(df$event_id))
print(paste('The last contest uploaded was Contest ', as.character(previous_contest), ', event_id ', last_event$event_id ,sep = ''))
```
Potential contests to upload:

```{r}
# previous_event_row <- events_played_to_date %>% filter(event_id == last_event$event_id) %>% pull(row_number)-1
# previous_event <- events_played_to_date %>% filter(row_number == previous_event_row)
# print(paste('The next event has event_id of ', previous_event$event_id, ', ', previous_event$event_name, sep = ''))


upcoming_events <- df_hist_current_year %>% filter(date >= last_event$date) %>% filter(event_id != last_event$event_id) %>% arrange(date)
upcoming_events
```
...
```{r}
year <- 2025 #for current year -> #as.numeric(format(Sys.Date(), "%Y"))
event <- 60
# selected_file <- '18_board.csv'
selected_file <- paste(as.character(previous_contest+1), '_board.csv', sep = '')
print(event)
print(selected_file)
